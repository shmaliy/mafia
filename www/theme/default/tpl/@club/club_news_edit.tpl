...
<div class="center_mod_title">Редактирование новости</div>
<?php else: ?>
<div class="center_mod_title">Добавление новости</div>
<?php endif; ?>
...
<div class="center_mod_body">
	<form name="create_news_form" action="#" method="post" enctype="multipart/form-data" onsubmit="return create(this,event);">
    <input type="hidden" name="function" value="club_news->newsSaveItem" />
	...
	<table class="form" cellpadding="0" cellspacing="0">
                ...
                <?php endif; ?>
                    <div class="icon"><img src="/image.php?i=<?php echo substr($this->item['image'], 1); ?>&t=png&m=fit&w=120&h=120&ca=cen&bg=eaeed7" />
                        <div class="edit_actions">
                            <a onclick="delImage(document.create_news_form.image.value);"><img src="/theme/default/img/delete.png" /></a>
                        </div>
                    </div>
                </div>
            </td>
		...
		<tr>
			<td colspan="3">Краткое описание (отображается только в списке)</td>
		</tr>
		<tr>
			<td colspan="3"><textarea name="introtext" style="height:150px;"><?php echo $this->item['introtext']; ?></textarea></td>
		</tr>
		<tr>
			<td colspan="3">Полный текст</td>
		</tr>
		<tr>
			<td colspan="3"><textarea name="fulltext"><?php echo $this->item['fulltext']; ?></textarea></td>
		</tr>
        <tr>
            <td colspan="2">&nbsp;</td>
            <td class="col3"><input id="create_news_form_submit" class="submit" style="width:110px;" type="submit" value="Сохранить" /></td>
        </tr>
		<tr>
			<td colspan="3"><div class="stat" id="create_news_stat"></div></td>
		</tr>
	</table>
    </form>
</div>
<script>
    ...
    {
        if ($("introtext_parent") || $("fulltext_parent")) {
            tinyMCE.triggerSave();
        }
        event.stop();
        ...
        new Ajax.Request('/index.php', {
            ...
            encoding: 'UTF-8',
            onCreate : (function(){
                document.create_news_form.title.setStyle({outline:"none"});
                $("uploadImage").setStyle({outline:"none"});
                $("introtext_parent").setStyle({outline:"none"}); // for mce editor field
                $("fulltext_parent").setStyle({outline:"none"}); // for mce editor field
                document.create_news_form.disable();
            }).bind(this),
    ...
    function delImage(img)
    {
        new Ajax.Request('/index.php', {
            method: 'post',
            parameters: {
                'function': 'club_news->newsDeleteImage',
    ...
    new SWFUpload({
        ...
        post_params : {
            'function': 'club_news->newsUploadImage',
            'itemId': '<?php echo $this->item['id']; ?>',
            'clubId': '<?php echo $this->club['id']; ?>',
            'catId': '<?php echo $this->cat['id']; ?>'
        },
