		...
		<div class="submod_body">
			<div class="photos">
				...
				<div class="photos_list_i">
					...
					<?php endif; ?>
                	<a href="<?php echo $linkItem; ?>"><img src="/image.php?i=<?php echo $image; ?>&t=png&m=fit&w=120&h=120&ca=cen&crop=true" /><div class="photos_list_i_title"><?php echo $item['title']; ?></div></a>
				</div>
				<?php endforeach; ?>
				<div class="clr"></div>
				<?php else: ?>
				<div class="empty">Нет фотоальбомов</div>
				<?php endif; ?>
			</div>
		</div>
    </div>
...
<script>
    function delItem(id, title)
    {
        ...
        if (confirm('Удалть Фотоальбом "' + title + '"?')) {
            new Ajax.Request('/index.php', {
                method: 'post',
                parameters: {
                    'function': 'club_photos->photosDeleteItem',
                    'itemId': id,
